    ...
    return (
        <main className="pt-20">
            ...
            <Section className="py-16 md:py-24">
                <div className="space-y-24 md:space-y-40">
                    {services.map((service, i) => (
                        <div key={service.id} className={`grid lg:grid-cols-2 gap-12 md:gap-20 items-center ${i % 2 !== 0 ? "lg:flex-row-reverse" : ""}`}>
                            <Reveal delay={0.1} className={i % 2 !== 0 ? "lg:order-2" : ""}>
                                <div className="relative aspect-[4/3] rounded-[40px] md:rounded-[60px] overflow-hidden group shadow-2xl">
                                    <Image
                                        src={service.image}
                                        alt={service.title}
                                        fill
                            ...
                            <div className={`space-y-8 md:space-y-10 text-center lg:text-left ${i % 2 !== 0 ? "lg:order-1" : ""}`}>
                                <Reveal>
                                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 text-primary text-[10px] font-black uppercase tracking-[0.2em] mb-4 lg:mb-0">
                                        Division 0{i + 1}
                                    </div>
                                    <h2 className="text-4xl md:text-7xl font-black text-heading tracking-tighter leading-[0.9] italic mt-4 lg:mt-6">
                                        {service.title}.
                                    </h2>
                                </Reveal>

                                <Reveal delay={0.1}>
                                    <p className="text-lg md:text-xl text-body leading-relaxed max-w-lg mx-auto lg:mx-0">
                                        {service.description}
                                    </p>
                                </Reveal>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 py-2 md:py-4">
                                ...
                                <Reveal delay={0.3}>
                                    <Link href={`/services/${service.id}`}>
                                        <Button className="w-full sm:w-auto h-14 md:h-16 px-10 rounded-full group">
                                            View Full details
            ...
            <Section className="pb-24 md:pb-32 px-4 md:px-6">
                <div className="bg-heading rounded-[40px] md:rounded-[60px] p-10 md:p-24 text-white text-center relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-8 md:space-y-12">
                        <Reveal>
                            <h2 className="text-4xl md:text-8xl font-black tracking-tighter italic leading-[0.9] md:leading-[0.8] max-w-4xl mx-auto">
                                READY TO EXPERIENCE <br />
                                <span className="text-primary">PRECISION CARE?</span>
                            </h2>
                        </Reveal>
                        <Reveal delay={0.1}>
                            <p className="text-lg md:text-xl text-white/50 max-w-xl mx-auto font-medium">
                                Fast results, world-class consultant radiologists, and a clinical environment designed for your comfort.
                            </p>
                        </Reveal>
                        <Reveal delay={0.2}>
                            <Button size="lg" className="w-full sm:w-auto rounded-full h-16 md:h-20 px-12 md:px-16 text-lg md:text-xl">
                                Book an Appointment
                            </Button>
                        </Reveal>
                    </div>
